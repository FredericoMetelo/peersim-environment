...
class PeersimEnv(ParallelEnv):
    ...
    def __init__(self, render_mode=None, simtype="basic", configs=None, log_dir=None, randomize_seed=False, preferred_port=8080,
        ...
        # TODO Have switch based on flag here!
    ...
    def observation_space(self, agent):
        # TODO: Will depend on the same flag as the definition of self._observation_spaces =
        return Dict(
            {
                STATE_NODE_ID_FIELD: Discrete(self.number_nodes, start=1),  # Ignores the controller
    ...
    def extract_local_data(self, partial_observation):
        Q = partial_observation[STATE_Q_FIELD]
        FS = partial_observation[STATE_FREE_SPACES_FIELD]
        processing_power = partial_observation[STATE_PROCESSING_POWER_FIELD]
        neighbours = partial_observation[STATE_NO_NEIGHBOURS]
        id = partial_observation[STATE_NODE_ID_FIELD]

        return Q, FS, processing_power, neighbours, id
