...
package PeersimSimulator.peersim.dynamics;

import PeersimSimulator.peersim.graph.*;
 ...
 * The properties the method has to fulfill are the following
 ...
 For PeersimSimulator.example, the class {@link WireWS} can be emulated by configuring
 <pre>
 init.0 WireByMethod
...
public WireByMethod(String prefix)
{
	...
	try
	{
		...
		Class.forName("PeersimSimulator.peersim.graph.GraphFactory"));
	...
	for(int i=1; i<args.length; ++i)
	{
		...
		else
		{
			...
			"report this problem to the PeersimSimulator.peersim team");
		}
	}
}
...
private static Method getMethod(Class cl, String methodName)
	...
	// it is a setter.
	final Class graphClass = Class.forName("PeersimSimulator.peersim.graph.Graph");
	...
	if( pars.length < 1 || !pars[0].isAssignableFrom(graphClass) )
		...
		" it has to have PeersimSimulator.peersim.graph.Graph as first argument type");
